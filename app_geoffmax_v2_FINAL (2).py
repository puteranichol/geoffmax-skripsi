...
# ══════════════════════════════════════════════════════════
    ...
    akurasi_cv_num = meta['akurasi'] if meta else 0.7966
    c1, c2 = st.columns(2)
    with c1: kpi_card(TEAL_LIGHT, "🎯", f"{akurasi_cv_num*100:.2f}%", "Akurasi Model (10-Fold CV)")
    with c2: kpi_card(BLUE_LIGHT, "📈", f"{auc_macro:.3f}", "AUC (thd data upload)")
    st.caption(
        "Catatan: confusion matrix, metrik per kelas, dan ROC curve di bawah ini dihitung dari "
        "model yang SUDAH dilatih (pipeline_geoffmax.pkl), diuji terhadap data yang baru diupload — "
        "bukan melatih ulang. Angka Akurasi di atas tetap memakai hasil 10-Fold CV resmi yang "
        "dilaporkan di Bab IV skripsi."
    )

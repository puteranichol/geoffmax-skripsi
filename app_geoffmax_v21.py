...
def buat_x_df(df):
    """Bentuk DataFrame input sesuai kolom yang diharapkan pipeline_final:
    ulasan_bersih, n_kata_positif, n_kata_negatif, fitur_campuran."""
    return df[['ulasan_bersih', 'n_kata_positif', 'n_kata_negatif', 'fitur_campuran']].copy()

# ══════════════════════════════════════════════════════════
# LOAD PIPELINE GABUNGAN (satu file, hasil Sel 6 notebook REVISI9)
# ══════════════════════════════════════════════════════════
@st.cache_resource
def load_model():
    diag = {'pipeline_geoffmax.pkl': 'ada di folder' if os.path.exists('pipeline_geoffmax.pkl') else 'TIDAK DITEMUKAN'}
    try:
        pipeline = joblib.load('pipeline_geoffmax.pkl')
        return pipeline, True, diag, None
...
meta      = load_meta()
pipeline_pkl, pkl_ada, pkl_diag, pkl_err = load_model()
...
with st.sidebar.expander("🔧 Diagnostik file .pkl", expanded=not pkl_ada):
    ...
    if pkl_err:
        ...
        st.success("pipeline_geoffmax.pkl berhasil dimuat ✅")
...
if uploaded:
    ...
    if pkl_ada:
        ...
        y_pred  = pipeline_pkl.predict(X_df)
        y_proba = pipeline_pkl.predict_proba(X_df)
    else:
        st.sidebar.error("File pipeline_geoffmax.pkl tidak ditemukan di repo — tidak bisa evaluasi.")
        st.stop()
...
# ══════════════════════════════════════════════════════════
    ...
    # Ambil TF-IDF yang sudah terlatih dari dalam pipeline (ColumnTransformer -> 'prep' -> 'tfidf')
    prep_step  = pipeline_pkl.named_steps['prep']
    tfidf_step = prep_step.named_transformers_['tfidf']

    section_start("Top 20 Kata — Bobot TF-IDF Tertinggi")
    fitur_names = tfidf_step.get_feature_names_out()
    X_all = tfidf_step.transform(df['ulasan_bersih'])
...
# ══════════════════════════════════════════════════════════
    ...
    c1, c2 = st.columns(2)
    with c1: kpi_card(TEAL_LIGHT, "🎯", f"{acc*100:.2f}%", "Akurasi pada Data Upload")
    with c2: kpi_card(BLUE_LIGHT, "📈", f"{auc_macro:.3f}", "AUC (Macro)")
    st.caption("Catatan: evaluasi ini memakai model yang SUDAH dilatih (pipeline_geoffmax.pkl), diuji terhadap data yang baru diupload — bukan melatih ulang.")
    ...
    prep_step  = pipeline_pkl.named_steps['prep']
    selector_step = pipeline_pkl.named_steps['select']
    model_step = pipeline_pkl.named_steps['nb']

    fitur_names_semua = prep_step.get_feature_names_out()
    fitur_names_terpilih = fitur_names_semua[selector_step.get_support()]

    fig_kata, axes = plt.subplots(1, 5, figsize=(24, 5))
    for cls_idx, (cls_name, color) in enumerate(zip(LABELS_TEXT, warna_roc)):
        log_prob = model_step.feature_log_prob_[cls_idx]
...
# ══════════════════════════════════════════════════════════
    ...
    if not pkl_ada:
        st.error("Model belum tersedia. Pastikan pipeline_geoffmax.pkl ada di repository.")
        st.stop()
    ...
    if klik:
        if teks_input.strip():
            ...
            if not teks_bersih.strip():
                ...
                pred_kelas = pipeline_pkl.predict(df_input)[0]
                pred_proba = pipeline_pkl.predict_proba(df_input)[0]

...
# ══════════════════════════════════════════════════════════
    ...
    # ────────────────────────────────────────────────────────
    # BAGIAN 2 — DEMO: EVALUASI TERHADAP DATA YANG DIUPLOAD
    # Ini BUKAN angka resmi Bab IV. Kalau CSV yang diupload sama/tumpang tindih
    # dengan data training, angkanya akan terlihat lebih tinggi (model "menghafal"
    # data itu) — jangan dipakai sebagai pengganti hasil CV di atas.
    # ────────────────────────────────────────────────────────
    st.subheader("🧪 Demo: Evaluasi terhadap Data yang Diupload")

    if not data_ada:
        st.info("Upload data CSV di sidebar untuk mencoba evaluasi interaktif terhadap data tersebut.")
        st.stop()

    acc_demo = accuracy_score(y_eval, y_pred)
    ...
    c1, c2 = st.columns(2)
    with c1: kpi_card(TEAL_LIGHT, "🎯", f"{acc_demo*100:.2f}%", "Akurasi pada CSV yang Diupload")
    with c2: kpi_card(BLUE_LIGHT, "📈", f"{auc_macro:.3f}", "AUC (thd data upload)")
    st.caption(
        "Catatan: bagian ini memakai model yang SUDAH dilatih (pipeline_geoffmax.pkl), diuji "
        "terhadap data yang baru diupload — bukan melatih ulang. Kalau angka di sini jauh lebih "
        "tinggi dari hasil resmi di atas, kemungkinan besar CSV yang diupload tumpang tindih "
        "dengan data training — gunakan data ulasan baru untuk hasil yang bermakna di sini."
    )

    st.markdown("<br>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        section_start("Confusion Matrix (Data Upload)")
    ...
    with col2:
        section_start("Metrik per Kelas (Data Upload)")
    ...
    section_start("ROC Curve (One-vs-Rest, 5 Kelas — Data Upload)")

...
if halaman == "📊 Beranda":
    ...
    if data_ada:
        ...
        if acc_upload > akurasi_cv_num + 0.03:
            st.warning(
                f"⚠️ Akurasi model terhadap CSV yang diupload ini adalah **{acc_upload*100:.2f}%** — "
                f"jauh lebih tinggi dari akurasi 10-Fold CV resmi ({akurasi_statis}) di atas. Ini "
                "kemungkinan besar karena CSV yang diupload sama (atau tumpang tindih besar) dengan "
                "data yang dipakai untuk melatih model, sehingga model 'menghafal' data ini, bukan "
                "benar-benar diuji ke data baru. Untuk evaluasi yang adil, upload data ulasan yang "
                "**belum pernah** dipakai saat training."
            )
        else:
            st.caption(f"Akurasi terhadap CSV yang diupload: {acc_upload*100:.2f}%.")
...
# ══════════════════════════════════════════════════════════
    ...
    if acc > akurasi_cv_num + 0.03:
        st.warning(
            f"⚠️ Akurasi ini ({acc*100:.2f}%) jauh lebih tinggi dari akurasi CV resmi "
            f"({akurasi_cv_num*100:.2f}%) — kemungkinan besar CSV yang diupload tumpang tindih dengan "
            "data training, sehingga model 'menghafal' data ini. Gunakan data ulasan baru yang belum "
            "pernah dipakai untuk training supaya angka ini benar-benar mengukur kemampuan generalisasi model."
        )

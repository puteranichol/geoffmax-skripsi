...
def interpretasi(teks):
    """Kotak penjelasan naratif, ditaruh DI DALAM section-card (kotak putih)
    tepat di bawah grafik/tabel/metrik terkait — permintaan revisi sidang."""
    st.markdown(f"""
...
with st.sidebar.expander("🔧 Diagnostik file .pkl", expanded=not pkl_ada):
    ...
    st.markdown("---")
    st.caption("File pendukung (Hasil Resmi & fitur leksikon):")
    meta_status = "ada di folder ✅" if os.path.exists('meta_geoffmax.pkl') else "TIDAK DITEMUKAN ❌"
    st.write(f"**meta_geoffmax.pkl**: {meta_status}")
    if meta is None:
        st.warning(
            "meta_geoffmax.pkl tidak berhasil dimuat — bagian 'Hasil Resmi (Bab IV)' di halaman "
            "Evaluasi Model akan tampil sebagai akurasi statis saja tanpa confusion matrix resmi. "
            "Jalankan Sel 6b di notebook, lalu upload file ini ke repo."
        )
    lek_status = "ada di folder ✅" if os.path.exists('leksikon_geoffmax.pkl') else "tidak ada (fallback: download InSet langsung)"
    sw_status  = "ada di folder ✅" if os.path.exists('stopwords_geoffmax.pkl') else "tidak ada (fallback: replikasi manual)"
    st.write(f"**leksikon_geoffmax.pkl**: {lek_status}")
    st.write(f"**stopwords_geoffmax.pkl**: {sw_status}")
...
if halaman == "📊 Beranda":
    ...
    if data_ada:
        ...
        interpretasi(
            ...
            "berhubungan secara statistik dengan tingkat kepuasan yang dipertahankan sebagai fitur model."
        )
        section_end()
        ...
        with col1:
            ...
            kelas_terbanyak = LABELS_TEXT[int(np.argmax(dist))]
            kelas_tersedikit = LABELS_TEXT[int(np.argmin(dist))]
            interpretasi(
                f"Grafik ini menunjukkan jumlah ulasan pada tiap kategori kepuasan (5 kelas sesuai revisi sidang). "
                f"Kelas dengan jumlah data terbanyak adalah <b>{kelas_terbanyak}</b> ({max(dist)} ulasan), "
                f"sedangkan yang paling sedikit adalah <b>{kelas_tersedikit}</b> ({min(dist)} ulasan). "
                "Jika sebarannya tidak merata (imbalanced), model bisa cenderung 'condong' memprediksi kelas "
                "yang datanya lebih banyak — inilah alasan pada tahap training digunakan teknik penyeimbangan data."
            )
            section_end()
        ...
        with col2:
            ...
            rating_terbanyak = int(rcnt.idxmax())
            interpretasi(
                "Grafik ini menunjukkan sebaran rating bintang (1–5) asli yang diberikan pelanggan di Shopee, "
                "sebelum dikonversi menjadi label kepuasan. "
                f"Rating <b>{rating_terbanyak} bintang</b> paling banyak muncul ({int(rcnt.max())} ulasan). "
                "Karena label kepuasan pada dashboard ini mengikuti rating asli 1–5 (hasil revisi sidang), "
                "distribusi rating di sini akan sejalan dengan distribusi kelas kepuasan pada grafik sebelah kiri."
            )
            section_end()
        ...
        interpretasi(
            "Tabel ini menampilkan contoh 10 ulasan pertama dari data yang diupload, lengkap dengan rating "
            "asli dari pelanggan dan label kepuasan (Y_Label) hasil konversi rating tersebut — untuk memastikan "
            "proses pelabelan berjalan sesuai aturan (rating 1 → Sangat Tidak Puas, ... rating 5 → Sangat Puas)."
        )
        section_end()
    else:
...
# ══════════════════════════════════════════════════════════
    ...
    interpretasi(
        f"Tabel ini merangkum data secara statistik: rata-rata rating pelanggan sekitar "
        f"<b>{df['rating'].mean():.2f}</b> dari skala 1–5, dan rata-rata panjang ulasan sekitar "
        f"<b>{df['panjang'].mean():.1f} kata</b>. Nilai <i>min</i> dan <i>max</i> membantu mengecek apakah "
        "ada data ganjil (misalnya ulasan kosong atau rating di luar rentang 1–5) sebelum data diproses lebih lanjut."
    )
    section_end()
    ...
    interpretasi(
        "Tabel ini membandingkan teks ulasan asli dengan hasil setelah <i>preprocessing</i> "
        "(case folding, penghapusan karakter non-huruf, penghapusan stopword, dan stemming). "
        "Tujuannya menyederhanakan teks tanpa menghilangkan kata-kata bermuatan sentimen penting "
        "(seperti kata negasi 'tidak', 'gak', atau kata intensitas 'banget') — kata-kata ini sengaja "
        "dipertahankan karena sangat berpengaruh terhadap tingkat kepuasan yang diprediksi."
    )
    section_end()
    ...
    interpretasi(
        f"Grafik ini menampilkan 20 kata dengan bobot <b>TF-IDF</b> (Term Frequency–Inverse Document Frequency) "
        f"tertinggi, dengan <b>'{top20.iloc[0]['Kata']}'</b> sebagai kata berbobot paling tinggi. "
        "Bobot TF-IDF tinggi berarti kata tersebut sering muncul di suatu ulasan tapi tidak terlalu umum "
        "di seluruh data — sehingga kata ini dianggap lebih 'khas' dan informatif untuk membedakan sentimen, "
        "dibanding kata yang muncul di hampir semua ulasan."
    )
    section_end()
    ...
    interpretasi(
        f"Histogram ini menunjukkan sebaran panjang ulasan (dalam jumlah kata) sebelum preprocessing. "
        f"Sebagian besar ulasan pelanggan GEOFFMAX berkisar di sekitar <b>{int(df['panjang'].median())} kata</b> "
        "(nilai tengah/median). Ulasan yang terlalu pendek biasanya minim informasi sentimen, sehingga pada "
        "tahap pelatihan model, ulasan yang setelah preprocessing tersisa sangat sedikit kata ikut disaring "
        "agar tidak mengganggu kualitas pembelajaran model."
    )
    section_end()
...
# ══════════════════════════════════════════════════════════
    ...
    # ────────────────────────────────────────────────────────
    # BAGIAN 1 — HASIL RESMI (dari meta_geoffmax.pkl, hasil 10-Fold CV di notebook)
    # Ini angka & confusion matrix yang SAMA PERSIS dengan yang dilaporkan di Bab IV
    # skripsi. Ditampilkan STATIS — tidak dihitung ulang dan tidak berubah walau
    # ada/tidaknya CSV yang diupload.
    # ────────────────────────────────────────────────────────
    st.subheader("📑 Hasil Resmi (Bab IV — 10-Fold Cross Validation)")

    if meta and 'confusion_matrix' in meta:
        akurasi_cv_num = meta['akurasi']
        n_cv = meta.get('n_data_cv', '—')
        c1, c2, c3, c4 = st.columns(4)
        with c1: kpi_card(TEAL_LIGHT, "🎯", f"{akurasi_cv_num*100:.2f}%", "Akurasi (10-Fold CV)")
        with c2: kpi_card(BLUE_LIGHT, "📈", f"{meta.get('auc_macro', 0):.3f}", "AUC (Macro)")
        with c3: kpi_card(ORANGE_LIGHT, "🧮", f"{meta.get('qwk', 0):.4f}", "QWK")
        with c4: kpi_card(PURPLE_LIGHT, "📝", f"{n_cv}", "Jumlah Data CV")

        section_start("Cara Membaca Metrik Evaluasi")
        interpretasi(
            f"<b>Akurasi {akurasi_cv_num*100:.2f}%</b> berarti dari seluruh data yang diuji dengan skema "
            "10-Fold Cross Validation (data dibagi 10 bagian, dilatih dan diuji bergantian agar hasil lebih stabil "
            f"dan tidak bergantung pada satu pembagian data saja), model berhasil memprediksi tingkat kepuasan "
            f"dengan benar sebanyak {akurasi_cv_num*100:.2f}% dari total data. "
            f"<b>AUC (Macro) {meta.get('auc_macro', 0):.3f}</b> mengukur kemampuan model membedakan antar "
            "kelas secara rata-rata di seluruh 5 kelas — semakin mendekati 1.0, semakin baik model memisahkan "
            "kelas satu dengan lainnya. "
            f"<b>QWK (Quadratic Weighted Kappa) {meta.get('qwk', 0):.4f}</b> dipakai karena label kepuasan di "
            "sini bersifat <i>ordinal</i> (berjenjang, dari Sangat Tidak Puas ke Sangat Puas) — QWK memberi "
            "'hukuman' lebih kecil untuk kesalahan prediksi yang jaraknya dekat (misal Puas ketuker Sangat Puas) "
            "dibanding kesalahan yang jaraknya jauh (misal Sangat Tidak Puas ketuker Sangat Puas). Nilai QWK "
            "yang tinggi menunjukkan model tetap 'masuk akal' meski akurasinya belum sempurna."
        )
        section_end()
        ...
        col1, col2 = st.columns(2)
        with col1:
            section_start("Confusion Matrix (Resmi)")
            cm_resmi = np.array(meta['confusion_matrix'])
            labels_resmi = meta.get('labels_text', LABELS_TEXT)
            fig_cm_resmi, ax = plt.subplots(figsize=(5.5, 4.5))
            biru_cmap = sns.light_palette(BLUE, as_cmap=True)
            sns.heatmap(cm_resmi, annot=True, fmt='d', cmap=biru_cmap,
                        xticklabels=labels_resmi, yticklabels=labels_resmi,
                        ax=ax, linewidths=0.5, annot_kws={'size': 11})
            ax.set_xlabel("Prediksi"); ax.set_ylabel("Aktual")
            ax.tick_params(axis='x', rotation=30)
            plt.tight_layout(); st.pyplot(fig_cm_resmi)

            cm_offdiag = cm_resmi.copy().astype(float)
            np.fill_diagonal(cm_offdiag, -1)
            r_idx, c_idx = np.unravel_index(np.argmax(cm_offdiag), cm_offdiag.shape)
            n_benar = int(np.trace(cm_resmi))
            n_total_cm = int(cm_resmi.sum())
            interpretasi(
                "Confusion matrix membandingkan label <b>Aktual</b> (baris) dengan hasil <b>Prediksi</b> model "
                "(kolom). Angka pada diagonal (dari kiri atas ke kanan bawah) adalah jumlah prediksi yang "
                f"<b>benar</b> — totalnya {n_benar} dari {n_total_cm} data. Angka di luar diagonal adalah "
                "kesalahan prediksi. Kesalahan paling sering terjadi antara kelas "
                f"<b>{labels_resmi[r_idx]}</b> yang justru diprediksi sebagai <b>{labels_resmi[c_idx]}</b> "
                f"({int(cm_resmi[r_idx, c_idx])} kasus) — ini wajar terjadi pada kelas-kelas 'tengah' "
                "yang teksnya cenderung ambigu secara bahasa (misalnya ulasan yang memuji sekaligus mengeluh)."
            )
            section_end()

        with col2:
            section_start("Metrik per Kelas (Resmi)")
            report_resmi = meta.get('classification_report')
            if report_resmi:
                auc_pc = meta.get('auc_per_class', [None] * len(labels_resmi))
                ring_resmi = pd.DataFrame({
                    'Kelas': labels_resmi,
                    'Precision': [report_resmi[l]['precision'] for l in labels_resmi],
                    'Recall': [report_resmi[l]['recall'] for l in labels_resmi],
                    'F1-Score': [report_resmi[l]['f1-score'] for l in labels_resmi],
                    'AUC (OvR)': auc_pc,
                    'Support': [int(report_resmi[l]['support']) for l in labels_resmi],
                })
                st.dataframe(ring_resmi.round(4), use_container_width=True, hide_index=True)
                idx_terendah = int(np.argmin(ring_resmi['F1-Score'].values))
                kelas_terendah = ring_resmi.iloc[idx_terendah]['Kelas']
                interpretasi(
                    "<b>Precision</b> = dari semua ulasan yang diprediksi masuk kelas tertentu, berapa persen "
                    "yang benar-benar termasuk kelas itu. <b>Recall</b> = dari semua ulasan yang aktualnya "
                    "kelas tertentu, berapa persen yang berhasil ditemukan model. <b>F1-Score</b> adalah "
                    "gabungan (rata-rata harmonis) dari precision dan recall. <b>Support</b> adalah jumlah data "
                    f"aktual di kelas tersebut. Kelas dengan F1-Score paling rendah adalah "
                    f"<b>{kelas_terendah}</b> — menandakan model paling kesulitan mengenali kelas ini secara "
                    "konsisten, biasanya karena jumlah datanya lebih sedikit atau teksnya lebih ambigu."
                )
            else:
                st.info("classification_report belum tersimpan di meta_geoffmax.pkl.")
            section_end()
    else:
        ...
        st.markdown(f"""
        <div style="background:{BLUE_LIGHT}; border-left:4px solid {BLUE}; border-radius:10px;
                    padding:10px 14px; margin:6px 0 14px 0; font-size:0.88rem; line-height:1.5; color:{TEXT_DARK};">
            💡 <b>Penjelasan:</b> Angka <b>{akurasi_statis}</b> adalah persentase prediksi yang benar dari hasil
            training model pada notebook, menggunakan skema label 5 kelas (Sangat Tidak Puas s.d Sangat Puas)
            sesuai rating asli 1–5. Detail confusion matrix dan metrik per kelas belum bisa ditampilkan di sini
            karena file pendukungnya belum lengkap (lihat peringatan di bawah).
        </div>
        """, unsafe_allow_html=True)
        st.warning(
            "`meta_geoffmax.pkl` belum berisi confusion matrix & classification report resmi "
            "(jalankan Sel 6b di notebook untuk mengekspornya). Menampilkan angka akurasi statis saja."
        )

    if pkl_ada:
        st.markdown("---")
        warna_kata = [WARNA[l] for l in LABELS_TEXT]
        ...
        for cls_idx, (cls_name, color) in enumerate(zip(LABELS_TEXT, warna_kata)):
        ...
        interpretasi(
            "Setiap panel menunjukkan 10 kata dengan <b>Log Probability</b> tertinggi menurut model Naïve Bayes "
            "untuk masing-masing kelas kepuasan. Log Probability yang tinggi berarti kata tersebut memiliki "
            "peluang besar muncul pada ulasan dari kelas itu. Contohnya, kata-kata di panel "
            f"<b>{LABELS_TEXT[0]}</b> biasanya berisi keluhan (misal 'kecewa', 'rusak', 'jelek'), sedangkan "
            f"kata-kata di panel <b>{LABELS_TEXT[-1]}</b> biasanya berisi pujian (misal 'nyaman', 'awet', 'puas'). "
            "Grafik ini membantu menjelaskan 'alasan' di balik prediksi model, bukan cuma menampilkan angka akurasi saja."
        )
        section_end()
    ...
    if klik:
        if teks_input.strip():
            ...
            if not teks_bersih.strip():
                ...
                interpretasi(
                    f"Model memprediksi ulasan ini paling mungkin masuk kelas <b>{label_hasil}</b> dengan "
                    f"probabilitas <b>{max(pred_proba)*100:.1f}%</b>. Grafik ini menunjukkan tingkat keyakinan "
                    "model terhadap kelima kemungkinan kelas kepuasan — semakin tinggi batang suatu kelas, "
                    "semakin yakin model bahwa ulasan tersebut termasuk kelas itu. Jika probabilitas antar dua "
                    "kelas berdekatan, artinya teks ulasan tersebut cenderung ambigu (mengandung sinyal campuran)."
                )
                section_end()
        else:
